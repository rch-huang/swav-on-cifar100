...
class ResNet(nn.Module):
    def __init__(
        ...
        # 对 CIFAR：3x3 conv，stride=1，padding=1；不做 maxpool
        ...
        # ---- 额外：SwAV-style 原型初始化，避免 logits 过大/过小 ----
        if isinstance(self.prototypes, nn.Linear):
            nn.init.normal_(self.prototypes.weight, mean=0.0, std=0.01)
        elif isinstance(self.prototypes, MultiPrototypes):
            for m in self.prototypes.modules():
    ...
    def forward_backbone(self, x):
        # 不再额外 padding，CIFAR 上直接 32x32 → 32x32
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)  # Identity for CIFAR
    ...
    def forward(self, inputs):
        # SwAV-style multi-crop interface
        if not isinstance(inputs, list):
            inputs = [inputs]

        # detect unique crop resolutions
        ...
        for end_idx in idx_crops:
            inp = torch.cat(inputs[start_idx:end_idx]).to(device, non_blocking=True)
            _out = self.forward_backbone(inp)
            if start_idx == 0:
                output = _out
            else:
...
def resnet18w2(**kwargs):
    return ResNet(BasicBlock, [2, 2, 2, 2], widen=2, **kwargs)


def resnet18w4(**kwargs):
    return ResNet(BasicBlock, [2, 2, 2, 2], widen=4, **kwargs)


def resnet50(**kwargs):
    # 如果你还需要 ResNet50，也可以保留
