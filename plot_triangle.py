...
def plot_cost_triangle(
    ...
    *,
    ...
    proto_order_by_task = {}
    for j in range(J):
        if j not in last_epoch:
            continue
        key = (j, j, last_epoch[j])
        if key not in file_map:
            continue
        d = load_npz(file_map[key])
        if "Q" not in d:
            raise KeyError(f"Missing Q in {file_map[key]} for proto order.")
        proto_order_by_task[j] = proto_order_from_qmass(d["Q"])
...
if __name__ == "__main__":
    ...
    out = plot_cost_triangle(
        log_dir=log_dir,
        out_png=log_dir+"./cost_triangle.png",
    )
    print("Saved:", out)
