...
def main():
    ...
    skip_controller = CurvatureSkipController(
        ...
        tau_curr_theta=0.4,            # -1 disable; >=0 enable
...
q_assign_test = {}
def train(train_loader, model, optimizer,scaler,task,_task, epoch, lr_schedule, queue,logfile='train_log.txt',sinkhorn_tracker=None,hessian_tracker = None, skip_controller=None,classids=[]):
    ...
    for it, inputs in enumerate(train_loader):
        ...
        if args.use_fp16:
            ...
            if skip_controller is not None:
                skip_blocks, skip_stats, rescale_plan = skip_controller.should_skip(
                    task=task,
                    epoch=epoch,
                    step=it,
                    model=model,
                        ...
                        if "theta" in skip_blocks and skip:
                            for name, p in model.named_parameters():
                                if "prototypes" not in name:
                                    p.grad = None
                    with open(logfile, "a") as f:
                        f.write(f"[CurvSkip]   iter={it} {skip_stats}\n")
                    #print(f"[CurvSkip]   iter={it} {skip_stats}")
                elif args.use_rescale != 0:
                    CurvatureSkipController.apply_rescale_plan(model, rescale_plan)
      
                    with open(logfile, "a") as f:
                        f.write(f"[CurvRescale]   iter={it} {skip_stats}\n")
                    #print(f"[CurvRescale]   iter={it} {skip_stats}")
